...
public class outboundCONCsort {
public static void main(String [] args) throws Exception {
        if (args.length!=2) {
            System.err.println("Usage: Outbound CONC sort <input path> <output path>");
            System.exit(-1);
         }
//telling JobTracker about the job.
        Job job = Job.getInstance();
        job.setJarByClass(outboundCONCsort.class);
        job.setJobName("Outbound Sort");
...
//set the Mapper and Reducer classes to be called.
        job.setMapperClass(outboundCONCsortMapper.class);
