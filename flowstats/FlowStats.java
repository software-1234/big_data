...
public class FlowStats {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length != 2) {
      System.err.println("Usage: FlowBytes <input path> <output path>");
      System.exit(-1);
    }
    //define the job to the JobTracker
    Job job = Job.getInstance();
    job.setJarByClass(FlowStats.class);
    job.setJobName("Flow Stats");
    ...
    job.setMapperClass(FlowStatsMapper.class);
    job.setReducerClass(FlowStatsReducer.class);
